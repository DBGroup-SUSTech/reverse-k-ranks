...
void ballTreeSearch(const int k, const float *qRow, const float qNorm, const int dim, BallTreeNode *node, vector<VectorElement> &heap);
...
void ballTreeSearch(const int k, const float *qRow, const float qNorm, const int dim, BallTreeNode *node,
	...
	if (node->isLeafNode()) {
		for (int index = 0; index < node->getSize(); index++) {
			...
			counter++;
#endif
            float innerProductValue = Calculator::innerProduct(node->getPtr(index), qRow, dim);
		...
		float leftIP = Calculator::innerProduct(node->getLeftNode()->getMean(), qRow, dim) +
		                node->getLeftNode()->getConstrain() * qNorm;
		float rightIP = Calculator::innerProduct(node->getRightNode()->getMean(), qRow, dim) +
...
inline void makeBallTree(BallTreeNode *node, Matrix &p, const int nodeSize) {
	...
	if (node->getSize() > nodeSize) {
		...
		const float* randomPointPtr = p.getRowPtr(node->getID(randomIndex));
		...
		for (int pointIndex = 0; pointIndex < node->getSize(); pointIndex++) {
			...
			const float* pointPtr = p.getRowPtr(node->getID(pointIndex));
		...
		const float* firstPointPtr = p.getRowPtr(firstPointID);
		...
		for (int pointIndex = 0; pointIndex < node->getSize(); pointIndex++) {
			...
			const float* pointPtr = p.getRowPtr(node->getID(pointIndex));
			float distance = Calculator::l2Distance(pointPtr, firstPointPtr, p.colNum);
		...
		vector<const float *> leftPointPtrs;
		vector<int> rightPointIDs;
		vector<const float *> rightPointPtrs;

		int secondPointID = node->getID(secondIDIndex);
		const float* secondPointPtr = p.getRowPtr(secondPointID);
		...
		for (int pointIndex = 0; pointIndex < node->getSize(); pointIndex++) {
			...
			const float *pointPtr = p.getRowPtr(pointID);
			float firstDis = Calculator::l2Distance(firstPointPtr, pointPtr, p.colNum);
			float secondDis = Calculator::l2Distance(secondPointPtr, pointPtr, p.colNum);
