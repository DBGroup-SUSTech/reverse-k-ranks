...
#include "BalltreeHeight.hpp"
#include "RtreeHeight.hpp"
...
void
WritePerformance(const std::vector<ReverseMIPS::TreeNodeInfo> &height_size_l,
    ...
    file << "height,n_element,size" << std::endl;
    for (int i = 0; i < arr_size; i++) {
        const int height = height_size_l[i].height;
        const int n_element = height_size_l[i].n_element;
        const double radius = height_size_l[i].radius;
        file << setiosflags(std::ios::fixed) << std::setprecision(3) << height << "," << n_element << "," << radius
             << std::endl;
    }
    file.close();
...
int main(int argc, char **argv) {
    ...
    TimeRecord record;
    record.reset();
    std::vector<TreeNodeInfo> data_item_height_size_l;
//    std::vector<std::pair<int, double>> user_height_size_l;
    if (method_name == "Balltree") {
        data_item_height_size_l = ReverseMIPS::BalltreeHeight::HeightSizeCurve(data_item);

    } else if (method_name == "Rtree") {
        data_item_height_size_l = ReverseMIPS::RtreeHeight::HeightSizeCurve(data_item);
//        user_height_size_l = ReverseMIPS::Rtree::HeightSizeCurve(user);
    ...
    const size_t node_size = data_item_height_size_l.size();
    const size_t n_skip = node_size / n_output;
    for (size_t i = 0; i < node_size; i += n_skip) {
        const TreeNodeInfo info = data_item_height_size_l[i];
        cout << "sample output " << i << ", height: " << info.height << ", # element in a node: " << info.n_element
             << ", radius: " << info.radius << endl;
    }
...
//                  [](std::pair<int, double> &p) { cout << "height: " << p.first << " size: " << p.second << endl; });
    WritePerformance(data_item_height_size_l, dataset_name, method_name.c_str(), "data_item");
