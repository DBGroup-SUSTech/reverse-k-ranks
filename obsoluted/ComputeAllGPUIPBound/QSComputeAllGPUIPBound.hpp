...
#ifndef REVERSE_KRANKS_QSCOMPUTEALLGPUIPBOUND_HPP
#define REVERSE_KRANKS_QSCOMPUTEALLGPUIPBOUND_HPP

#include "util/NameTranslation.hpp"

#include "alg/SpaceInnerProduct.hpp"
#include "alg/TopkMaxHeap.hpp"
#include "ComputeAllGPUIPBound.hpp"
...
namespace ReverseMIPS::QSComputeAllGPUIPBound {
    ...
    class Index : public BaseIndex {
        ...
        //read disk
        ComputeAllGPUIPBound disk_ins_;
    ...
    public:
        ...
        Retrieval(const VectorMatrix &query_item, const int &topk, const int &n_execute_query,
            ...
            for (int queryID = 0; queryID < n_query_item; queryID++) {
                ...
                disk_ins_.ComputeQueryIP(query_vecs, queryIP_l_.data());
        ...
        PerformanceStatistics(const int &topk) override {
            ...
            sprintf(buff,
                    "top%d retrieval time: total %.3fs\n\tinner product %.3fs, rank bound %.3fs, prune user %.3fs, refine user %.3fs\n\ttotal ip cost %ld, total refine ip cost %ld, sample refine user %ld, total refine user %ld, rank prune ratio %.4f, average compute item %.3f",
    ...
    BuildIndex(VectorMatrix &data_item, VectorMatrix &user, const char *index_path, const char *dataset_name,
        ...
        const std::string index_name = IndexName("QSComputeAllGPUIPBound");
        //rank search
        SampleSearch rank_ins(index_basic_dir, dataset_name, index_name.c_str(),
                              n_sample, true, true,
                              n_sample_query, sample_topk);

        ComputeAllGPUIPBound disk_ins(user, data_item);
...
#endif //REVERSE_KRANKS_QSCOMPUTEALLGPUIPBOUND_HPP
