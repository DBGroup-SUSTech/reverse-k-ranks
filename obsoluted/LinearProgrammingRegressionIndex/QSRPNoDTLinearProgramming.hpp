...
#include "UniformLinearRegression.hpp"
...
namespace ReverseMIPS::QSRPNoDT {
    ...
    BuildIndex(VectorMatrix &data_item, VectorMatrix &user, const char *dataset_name,
        ...
        const std::string index_name = IndexName("QSRPNoDT");
        //rank search
        SampleSearch rank_ins(index_basic_dir, dataset_name, index_name.c_str(),
                              n_sample, true, true,
                              n_sample_query, sample_topk);

        UniformLinearRegression rank_bound_ins(index_basic_dir, dataset_name, "QSRPNoDT", n_sample, n_sample_query,
                                               sample_topk);
