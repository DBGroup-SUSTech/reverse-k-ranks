...
//                args.threads = omp_get_max_threads();
        ...
        virtual void GetRank(const std::vector<float> &queryIP_l,
            ...
            record_.reset();
#pragma omp parallel for default(none) shared(is_continue, prune_l, result_l, rank_ub_l, queryIP_l, refine_ip_cost, n_compute_item, n_candidate, n_result_rank_ub, rank_lb_l, n_remain_result)
            for (int userID = 0; userID < n_user_; userID++) {
                ...
                if (rank_lb_l[userID] == rank_ub_l[userID]) {
                    ...
                    algo_ins_l_[threadID].runAboveTheta(user_lemp_l_[userID], n_refine_ip, rank);
                }
