...
namespace mips {
    ...
    public:
        ...
        inline void runTopK(const float* query, ProbeBucket& probeBucket, RetrievalArguments* arg) const {
            ...
            float localTheta = arg->heap.front().data;
...
//            std::cout<<"arg->currEpsilonAppr: "<<arg->currEpsilonAppr<<std::endl;
            ...
            if (shouldScan) {
                ...
                if (validLists == 1) {
                    ...
                    for (row_type i = 0; i < length; ++i) {
                        row_type row = entry[i].id;
                        float len = arg->probeMatrix->lengthInfo[row + probeBucket.startPos].data;
                        float privTheta = arg->heap.front().data;
#ifdef RELATIVE_APPROX
        ...
        inline void scanOnly1ListTopk(const float* query, ProbeBucket& probeBucket, RetrievalArguments* arg,
                QueueElementLists* invLists, bool topk) const {
#ifdef TIME_IT
            arg->t.start();
#endif          
