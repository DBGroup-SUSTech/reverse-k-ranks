...
//                timeX[numLists - 1].emplace_back(retrArg[0].tunerTimer.elapsedTime().nanos() + preprocessTime);
        ...
        template<typename R>
        void tuneTopk(ProbeBucket &probeBucket, const ProbeBucket &prevBucket, std::vector<RetrievalArguments> &retrArg,
                      R *retriever) {
            upperActive = true;
            lowerActive = true;
            ...
            float previousForUpper = bestTimeForPhi[list - 1];
            float previousForLower = bestTimeForPhi[list - 1];
            ...
            for (col_type i = 1; i <= tries; ++i) {
                ...
                if (upperActive) {
                    tuneBucketForListTopk(probeBucket, prevBucket, retrArg, list + i, previousForUpper, true,
                                          retriever);
                    previousForUpper = bestTimeForPhi[list + i - 1];
                }
                if (lowerActive) {
                    tuneBucketForListTopk(probeBucket, prevBucket, retrArg, list - i, previousForLower, false,
                                          retriever);
