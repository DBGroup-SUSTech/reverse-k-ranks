...
namespace mips {
    ...
    public:
        ...
        inline virtual void initForNewQuery(const float* q) {
            query = q;
            exploredItems.reset();
            allSeen = false;
        }
        ...
        inline float getInitialThreshold() const {
        ...
        inline bool isThresholdUnterTheta(float& stopThreshold, float localTheta, col_type stepOnCol, float oldValue, bool forCosine) const {
        ...
        inline void maintainExploredLists(const VectorMatrixLEMP& matrix, row_type countSeen, col_type stepOnCol) {
            if (countSeen == rowNum) {
                allSeen = true;
            } else {
