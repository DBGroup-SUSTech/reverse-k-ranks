...
namespace mips {
    ...
    typedef boost::numeric::ublas::matrix<float, boost::numeric::ublas::row_major> DenseMatrix; // for vector matrix
    typedef boost::numeric::ublas::matrix<float, boost::numeric::ublas::column_major> DenseMatrixCM; // for lists
    ...
    struct MatItem {
        ...
        inline MatItem(float result, ta_size_type i, ta_size_type j) : result(result), i(i), j(j) {
        };
    ...
    struct QueueElement {
        ...
        inline QueueElement(float data, ta_size_type id) : data(data), id(id) {
        };
    ...
    public:
        ...
        inline void updateRoot(float newData) {
            heap[0].data = newData;
            heapify(0);
        }
    ...
    */
    inline void calculateTimeInCutoff(const std::vector<float>& method1, const std::vector<float>& method2, row_type cutoffSample, float& time) {
        if (cutoffSample == 0) {
            time = 0;
    ...
    inline void findCutOffPoint(const std::vector<float>& method1, const std::vector<float>& method2, float& bestTime, row_type& best_t_b_ind) {
        float time;
    ...
    inline void findCutOffPointInv(const std::vector<float>& method1, const std::vector<float>& method2, float& bestTime, row_type& best_t_b_ind) {
        float time = bestTime;
