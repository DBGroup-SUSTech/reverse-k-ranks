...
*/
    namespace details {
        ...
        */
        template<typename T>
        inline T distance_squared(const std::vector<T> &point_a, const std::vector<T> &point_b) {
        ...
        template<typename T>
        inline T distance(const std::vector<T> &point_a, const std::vector<T> &point_b) {
        ...
        */
        ...
        random_plusplus(const std::vector<std::vector<T>> &data, uint32_t k, uint64_t seed) {
            assert(k > 0);
            assert(data.size() > 0);
...
*/
        ...
        std::vector<std::vector<T>> calculate_means(const std::vector<std::vector<T>> &data,
                                                    ...
                                                    uint32_t k) {
            const unsigned int dimension = data[0].size();
            std::vector<std::vector<T>> means(k, std::vector<T>(dimension));
