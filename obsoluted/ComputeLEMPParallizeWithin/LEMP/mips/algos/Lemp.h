...
//            dataPreprocessingTimeRight += record.get_elapsed_time_second() * 1e9;
        ...
        inline void runAboveTheta(VectorMatrixLEMP &leftMatrix, size_t &n_compare, int &n_result) {
...
#pragma omp parallel default(none) shared(n_result) reduction(+ : comparisons)
            {
...
//                results.moveAppend(retrArg[tid].results, tid);
                n_result += retrArg[tid].results.size();
            }
...
//        std::cout << "[RETRIEVAL] ProbeBuckets (active) = " << activeBuckets << std::endl;
        ...
        switch (args.method) {
            case LEMP_LI:
            case LEMP_I:
#pragma omp parallel for default(none) shared(b0) schedule(dynamic, 1)
                for (row_type b = b0; b < activeBuckets; ++b) {
...
#pragma omp parallel for default(none) shared(b0) schedule(dynamic, 1)
                for (row_type b = b0; b < activeBuckets; ++b) {
                    static_cast<IntLists *> (probeBuckets[b].ptrIndexes[INT_SL])->initializeLists(probeMatrix,
                                                                                                  probeBuckets[b].startPos,
                                                                                                  probeBuckets[b].endPos);
                }
                break;
...
#pragma omp parallel for default(none) shared(retrArg) schedule(dynamic, 1)
