...
namespace mips {
    ...
    inline row_type computeBlockOffsetsByFactorCacheFittingForItems(const std::vector<QueueElement>& elements, row_type size,
            std::vector<row_type>& blockOffsets, float factor, row_type minItems, row_type cacheSizeInKB, row_type rank,
            const LempArguments& args) {

        int doubleSize = 4; //bytes
        ...
        switch (args.method) {
            ...
            case LEMP_TREE:
                //singleVectorSpace *= 2; // in reality I have no idea how much space the tree will take but here I try to float the space
        ...
        while (blockOffsets[blockOffsets.size() - 1] < size) {
            float value = elements[blockOffsets[blockOffsets.size() - 1]].data * factor;
    ...
    inline void computeBlockOffsetsForUsersFixed(row_type size, std::vector<row_type>& blockOffsets,
        ...
        int blocks = ceil((float) size / maxItems);
    ...
    template<typename T>
    inline void bucketize(std::vector<T>& buckets, const VectorMatrixLEMP& matrix,
