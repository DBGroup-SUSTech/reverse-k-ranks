...
#ifndef REVERSE_KRANKS_QSCOMPUTEALLIPBOUND_HPP
#define REVERSE_KRANKS_QSCOMPUTEALLIPBOUND_HPP
...
namespace ReverseMIPS::QSComputeAllIPBound {
...
#pragma omp parallel for default(none) shared(queryIP_l_, query_vecs, svd_ins_) num_threads(omp_get_num_procs()) schedule(dynamic, 1000)
                for (int userID = 0; userID < n_user_; userID++) {
                    if (prune_l_[userID] || result_l_[userID]) {
                        continue;
                    }
                    queryIP_l_[userID] = InnerProduct(query_vecs, svd_ins_.transfer_user_.getVector(userID), vec_dim_);
                }
    ...
    BuildIndex(const VectorMatrix &data_item, const VectorMatrix &user, const char *dataset_name,
        ...
        //rank search
        const std::string index_name = IndexName("QSComputeAllIPBound");
        ...
        std::unique_ptr<Index> index_ptr = std::make_unique<Index>(svd_ins, rank_ins, disk_ins,
                                                                   user.n_vector_, data_item.n_vector_, user.vec_dim_);
        return index_ptr;
    }

}

#endif //REVERSE_KRANKS_QSCOMPUTEALLIPBOUND_HPP
