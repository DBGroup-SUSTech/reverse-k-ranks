...
namespace dyy {
    ...
    const float SPLIT_FACTOR = 0.4;
    const float REINSERT_FACTOR = 0.3;
 ...
 *****************************************************************************/
    ...
    public:
        ...
        inline float getCenter(size_t dim) { return (coord[dim][0] + coord[dim][1]) * 0.5; }

        static Mbr getMbr(Mbr &mbr1, Mbr &mbr2);

        static float getOverlap(Mbr &mbr1, Mbr &mbr2);

        static float getEnlarge(Mbr &base, Mbr &add);
    };
