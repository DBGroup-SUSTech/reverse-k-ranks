...
namespace mips {
    ...
    protected:
        ...
        std::unordered_map<row_type, std::pair<float, std::vector<col_type> > > worstScores;
    ...
    public:
        ...
        inline virtual void initForNewQuery(const float* q) {// initialize to specific rows
            inactiveCols = 0;
            query = q;
        }
        ...
        inline void getCandidates(row_type* candidatesToVerify, row_type& numCandidatesToVerify, float localTheta, float stopThreshold) {
            std::unordered_map<row_type, std::pair<float, std::vector<col_type> > >::iterator it;

            for (it = worstScores.begin(); it != worstScores.end(); it++) {
                row_type key = it->first;
                float ws = it->second.first;
        ...
        inline float getInitialThreshold() {
        ...
        inline bool isThresholdUnterTheta(float& stopThreshold, float localTheta, col_type stepOnCol, float oldValue, bool forCosine) {

            if (colNum <= inactiveCols) {
                return true;
            }
            float piNew = invLists->getElement(fringePos[stepOnCol])->data;
    ...
    public:
        ...
        inline void initForNewQuery(const float* q) {// initialize to specific rows
        ...
        inline virtual void updateState(col_type col) {// given that the step happened on col update the structures
            ...
            if (inactivate) {
                piqi.updateRoot((-1) * std::numeric_limits<float>::max());
                inactiveCols++;
            } else {
