...
namespace mips {
    ...
    class Candidate_incr {
        ...
        //         float x[2] __attribute__((aligned(16))); //x[0]: sum pi*qi x[1]: sum pi^2

        __m128d x;
#else
        float x[2]; //x[0]: sum pi*qi x[1]: sum pi^2        
#endif
    ...
    public:
        ...
        inline void addFirst(float qi, float pi) {
        ...
        inline void add(float qi, float pi) {
        ...
        inline bool prune(float len, float theta, float seenQi2) {


#ifdef  WITH_SIMD_INCR

            float x_tmp[2] __attribute__((aligned(16)));
