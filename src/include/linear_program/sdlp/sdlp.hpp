...
namespace sdlp {
    ...
    inline float dot2(const float a[2],
                      const float b[2]) {
        return a[0] * b[0] + a[1] * b[1];
    }

    inline float cross2(const float a[2],
                        const float b[2]) {
        return a[0] * b[1] - a[1] * b[0];
    }

    inline bool unit2(const float a[2],
                      float b[2]) {
        const float mag = std::sqrt(a[0] * a[0] +
                                    a[1] * a[1]);
        if (mag < 2.0 * eps) {
            return true;
        }
    ...
    template<int d>
    inline int lp_no_con(const float *n_vec,
                         const float *d_vec,
                         float *opt) {
        float n_dot_d = 0.0;
        float d_dot_d = 0.0;
    ...
    inline void lp_min_lin_rat(const bool degen,
                               const float cw_vec[2],
                               const float ccw_vec[2],
                               const float n_vec[2],
                               const float d_vec[2],
                               float opt[2]) {
        /* linear rational function case */
        const float d_cw = dot2(cw_vec, d_vec);
        const float d_ccw = dot2(ccw_vec, d_vec);
        const float n_cw = dot2(cw_vec, n_vec);
        const float n_ccw = dot2(ccw_vec, n_vec);
        if (degen) {
    ...
    inline int wedge(const float (*halves)[2],
                     const int m,
                     int *next,
                     int *prev,
                     float cw_vec[2],
    ...
    inline int lp_base_case(const float (*halves)[2], /* halves --- half lines */
                            const int m,               /* m      --- terminal marker */
                            const float n_vec[2],     /* n_vec  --- numerator funciton */
                            const float d_vec[2],     /* d_vec  --- denominator function */
                            float opt[2],             /* opt    --- optimum  */
                            int *next,                 /* next, prev  --- float linked list of indices */
                            int *prev) {
    ...
    template<int d>
    inline void findimax(const float *pln,
                         int *imax) {
        *imax = 0;
        float rmax = std::fabs(pln[0]);
        for (int i = 1; i <= d; i++) {
            const float ab = std::fabs(pln[i]);
            if (ab > rmax) {
                *imax = i;
                rmax = ab;
            }
    ...
    template<int d>
    inline void vector_up(const float *equation,
                          const int ivar,
                          const float *low_vector,
                          float *vector) {
        vector[ivar] = 0.0;
        for (int i = 0; i <= d; i++) {
            if (i != ivar) {
    ...
    template<int d>
    inline void vector_down(const float *elim_eqn,
                            const int ivar,
                            const float *old_vec,
                            float *new_vec) {
        float ve = 0.0;
    ...
    template<int d>
    inline void plane_down(const float *elim_eqn,
                           const int ivar,
                           const float *old_plane,
                           float *new_plane) {
        const float crit = old_plane[ivar] / elim_eqn[ivar];
        for (int i = 0; i <= d; i++) {
            if (i != ivar) {
    ...
    template<int d>
    inline int linfracprog(const float *halves, /* halves  --- half spaces */
                           const int max_size,   /* max_size --- size of halves array */
                           const int m,          /* m       --- terminal marker */
                           const float *n_vec,  /* n_vec   --- numerator vector */
                           const float *d_vec,  /* d_vec   --- denominator vector */
    ...
    */
    {
        ...
        for (int i = 0; i != m; i = next[i]) {
            ...
            if (val < -(d + 1) * eps) {
                ...
                if (i != 0) {
                    const float fac = 1.0f / plane_i[imax];
                    for (int j = 0; j != i; j = next[j]) {
                        const float *old_plane = halves + j * (d + 1);
                        const float crit = old_plane[imax] * fac;
                        float *new_plane = new_halves + j * d;
                        for (int k = 0; k <= d; k++) {
    ...
    template<>
    inline int linfracprog<1>(const float *halves,
                              const int max_size,
                              const int m,
                              const float *n_vec,
                              const float *d_vec,
                              ...
                              int *prev) {
        if (m > 0) {
            return lp_base_case((const float (*)[2]) halves, m,
...
//    }
    ...
    template<int d>
    inline float linprog(const Eigen::Matrix<float, d, 1> &c,
                         const Eigen::Matrix<float, -1, d> &A,
                         const Eigen::Matrix<float, -1, 1> &b,
                         const int *b_random_permutation,
...
//        printf("1\n");
        ...
        Eigen::Matrix<float, d + 1, 1> n_vec;
        Eigen::Matrix<float, d + 1, 1> d_vec;
        Eigen::Matrix<float, d + 1, 1> opt;
        Eigen::Matrix<float, d + 1, -1, Eigen::ColMajor> halves(d + 1, m);
        Eigen::VectorXf work((m + 3) * (d + 2) * (d - 1) / 2);
...
//        rand_permutation(m - 1, perm.data());
        std::memcpy(perm.data(), b_random_permutation, (m - 1) * sizeof(int));
