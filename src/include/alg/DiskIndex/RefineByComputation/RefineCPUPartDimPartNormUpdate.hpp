...
#ifndef REVERSE_KRANKS_REFINECPUPARTDIMPARTNORMUPDATE_HPP
#define REVERSE_KRANKS_REFINECPUPARTDIMPARTNORMUPDATE_HPP
...
namespace ReverseMIPS {
    ...
    class RefineCPUPartDimPartNormUpdate {
    ...
    public:
        ...
        inline RefineCPUPartDimPartNormUpdate() = default;

        inline RefineCPUPartDimPartNormUpdate(const int &n_user, const int &n_data_item, const int &vec_dim,
                                              const int &check_dim) {
            this->n_user_ = n_user;
        ...
        void Preprocess(const VectorMatrixUpdate &user, const VectorMatrixUpdate &data_item) {
        ...
        void
        IPBound(const VectorMatrixUpdate &data_item, const int &n_proc_item, const float *user_vecs, const int &userID,
...
#pragma omp parallel for default(none) shared(n_proc_item, data_item, user_vecs, itemIP_l, checkdimIP_l, user_norm)
        ...
        void
        IPBoundNoParallel(const VectorMatrixUpdate &data_item, const int &n_proc_item, const float *user_vecs,
                          const int &userID,
                          std::vector<std::pair<float, float>> &itemIP_l, std::vector<float> &checkdimIP_l) const {
...
//#pragma omp parallel for default(none) shared(n_proc_item, data_item, user_vecs, itemIP_l, checkdimIP_l, user_norm)
        ...
        void ComputeRemainDim(const VectorMatrixUpdate &data_item, const int &n_proc_item,
...
#pragma omp parallel for default(none) shared(n_proc_item, data_item, user_check_vecs, is_rank_larger_l, is_rank_lower_l, queryIP_l)
        ...
        void ComputeRemainDimNoParallel(const VectorMatrixUpdate &data_item, const int &n_proc_item,
                                        const float *user_vecs,
...
#endif //REVERSE_KRANKS_REFINECPUPARTDIMPARTNORMUPDATE_HPP
