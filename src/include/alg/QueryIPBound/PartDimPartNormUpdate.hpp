...
#pragma omp parallel for default(none) shared(n_proc_user, user, query_transfer_vecs, query_transfer_norm, queryIP_l) num_threads(n_thread_)
...
#pragma omp parallel for default(none) shared(n_proc_user, prune_l, result_l, user, query_transfer_vecs, queryIP_l) num_threads(n_thread_)
            for (int userID = 0; userID < n_proc_user; userID++) {
                ...
                const float *query_check_vecs = query_transfer_vecs + check_dim_;
