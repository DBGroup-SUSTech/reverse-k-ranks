...
#ifndef REVERSE_KRANKS_NORMALLINEARREGRESSIONLPUPDATE_HPP
#define REVERSE_KRANKS_NORMALLINEARREGRESSIONLPUPDATE_HPP
...
namespace ReverseMIPS {
    ...
    class NormalLinearRegressionLPUpdate {
    ...
    public:
        ...
        inline NormalLinearRegressionLPUpdate() {}

        inline NormalLinearRegressionLPUpdate(const char *index_basic_dir, const char *dataset_name,
        ...
        void InsertUser(const VectorMatrixUpdate &insert_user, const std::vector<const float *> &sampleIP_l_l) {
            StartPreprocess();

            std::unique_ptr<float[]> new_predict_para_l = std::make_unique<float[]>(
                    (n_user_ + insert_user.n_vector_) * n_predict_parameter_);
            std::memcpy(new_predict_para_l.get(), predict_para_l_.get(),
                        n_user_ * n_predict_parameter_ * sizeof(float));
            predict_para_l_ = std::move(new_predict_para_l);

            std::unique_ptr<float[]> new_distribution_para_l = std::make_unique<float[]>(
                    (n_user_ + insert_user.n_vector_) * n_distribution_parameter_);
            std::memcpy(new_distribution_para_l.get(), distribution_para_l_.get(),
                        n_user_ * n_distribution_parameter_ * sizeof(float));
            distribution_para_l_ = std::move(new_distribution_para_l);

            std::unique_ptr<float[]> new_error_l = std::make_unique<float[]>(
                    n_user_ + insert_user.n_vector_);
            std::memcpy(new_error_l.get(), error_l_.get(),
                        n_user_ * sizeof(float));
            error_l_ = std::move(new_error_l);

            std::unique_ptr<float[]> new_sample_score_extreme_l = std::make_unique<float[]>(
                    (n_user_ + insert_user.n_vector_) * 2);
            std::memcpy(new_sample_score_extreme_l.get(), sample_score_extreme_l_.get(),
                        n_user_ * 2 * sizeof(float));
            sample_score_extreme_l_ = std::move(new_sample_score_extreme_l);

            const int n_batch =
                    insert_user.n_vector_ / batch_n_user_ + insert_user.n_vector_ % batch_n_user_ == 0 ? 0 : 1;
            for (int batchID = 0; batchID < n_batch; batchID++) {
                const int start_insert_userID = batchID * batch_n_user_;
                const int n_proc_user = std::min(batch_n_user_, insert_user.n_vector_ - start_insert_userID);

#pragma omp parallel for default(none) shared(start_insert_userID, n_proc_user, sampleIP_l_l, n_sample_rank_, distribution_para_l_) num_threads(omp_get_max_threads())
                for (int proc_userID = 0; proc_userID < n_proc_user; proc_userID++) {
                    const float *sampleIP_l = sampleIP_l_l[start_insert_userID + proc_userID];
                    float *preprocess_cache_A = batch_cache_A_l_[proc_userID];
                    const int userID = proc_userID + start_insert_userID + n_user_;
                    AssignCache(preprocess_cache_A, sampleIP_l, userID);
                }

#pragma omp parallel for default(none) shared(n_proc_user, start_insert_userID, sampleIP_l_l, n_sample_rank_, predict_para_l_, batch_cache_A_l_, preprocess_cache_b_, preprocess_cache_c_, preprocess_b_random_permutation_) num_threads(omp_get_max_threads())
                for (int proc_userID = 0; proc_userID < n_proc_user; proc_userID++) {
...
#pragma omp parallel for default(none) shared(sampleIP_l_l, n_proc_user, n_sample_rank_, predict_para_l_, error_l_, start_insert_userID, distribution_para_l_)
                for (int proc_userID = 0; proc_userID < n_proc_user; proc_userID++) {
                    const int userID = proc_userID + start_insert_userID + n_user_;
                    if (error_l_[userID] < 0 || error_l_[userID] > n_sample_rank_ - 1e-3) {
                        ...
                        const float last_sampleIP = CDFPhi(
                                (sampleIP_l[n_sample_rank_ - 1] - mu) / sigma);
        ...
        void DeleteUser(const std::vector<int> &del_userID_l) {
            ...
            for (int userID = 0; userID < n_user_; userID++) {
                if (del_userID_set.find(userID) == del_userID_set.end()) {
                    remain_userID_l.emplace_back();
                }
            }

            std::unique_ptr<float[]> new_predict_para_l = std::make_unique<float[]>(
                    remain_userID_l.size() * n_predict_parameter_);
            std::unique_ptr<float[]> new_distribution_para_l = std::make_unique<float[]>(
                    remain_userID_l.size() * n_distribution_parameter_);
            std::unique_ptr<float[]> new_error_l = std::make_unique<float[]>(remain_userID_l.size());
            std::unique_ptr<float[]> new_sample_score_extreme_l = std::make_unique<float[]>(remain_userID_l.size() * 2);

            for (int candID = 0; candID < remain_userID_l.size(); candID++) {
                const int userID = remain_userID_l[candID];
                std::memcpy(new_predict_para_l.get() + candID * n_predict_parameter_,
                            predict_para_l_.get() + userID * n_predict_parameter_,
                            n_predict_parameter_ * sizeof(float));
                std::memcpy(new_distribution_para_l.get() + candID * n_distribution_parameter_,
                            distribution_para_l_.get() + userID * n_distribution_parameter_,
                            n_distribution_parameter_ * sizeof(float));
                std::memcpy(new_error_l.get() + candID, error_l_.get() + userID, sizeof(float));
                std::memcpy(new_sample_score_extreme_l.get() + candID * 2,
                            sample_score_extreme_l_.get() + userID * 2, 2 * sizeof(float));
            }
        ...
        void StartPreprocess() {
            ...
            for (int batchID = 0; batchID < batch_n_user_; batchID++) {
                batch_cache_A_l_[batchID] = new float[(4 * (n_sample_rank_ - 1)) * (n_predict_parameter_ + 1)];
            }

            this->preprocess_cache_b_ = new float[4 * (n_sample_rank_ - 1)];
            this->preprocess_cache_c_ = new float[3];
#pragma omp parallel for default(none) shared(sample_rank_l_)
            ...
            this->preprocess_b_random_permutation_ = new int[4 * (n_sample_rank_ - 1)];
            std::iota(preprocess_b_random_permutation_, preprocess_b_random_permutation_ + 4 * (n_sample_rank_ - 1), 0);
            std::shuffle(preprocess_b_random_permutation_, preprocess_b_random_permutation_ + 4 * (n_sample_rank_ - 1),
...
#endif //REVERSE_KRANKS_NORMALLINEARREGRESSIONLPUPDATE_HPP
