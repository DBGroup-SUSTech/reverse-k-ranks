...
{\
  ...
  {\
    ...
    struct sort_functor {
        ...
        sort_functor(thrust::device_ptr<float> data, size_t dsize) {
            data_ = data;
            dsize_ = dsize;
        }
        ...
        void operator()(const int start_idx) {
            thrust::sort(thrust::device, data_ + (dsize_ * start_idx), data_ + (dsize_ * (start_idx + 1)),
...
//    return std::min((int) batch_n_user, 128);
        ...
        void ComputeQueryRank(const int &start_userID, const int &n_compute_user, int *sample_item_rank_ptr) {
            ...
            ComputeList(start_userID, n_compute_user, (int) n_data_item_, item_device_ptr_,
                        score_table_device_ptr_);
            printf("compute list\n");
            const double compute_table_time = record.get_elapsed_time_second();

            record.reset();
            SortList(n_compute_user);
            printf("sort list\n");
            const double sort_table_time = record.get_elapsed_time_second();

            record.reset();
            ComputeList(start_userID, n_compute_user, (int) n_sample_item_, sample_item_device_ptr_,
                        sample_item_IP_device_ptr_);
            printf("compute list2\n");
            const double compute_query_IP_time = record.get_elapsed_time_second();

            // sort the query rank
            record.reset();
            ComputeRankOfQuery(n_compute_user);
            printf("compute rank of query\n");
            const double compute_query_rank_time = record.get_elapsed_time_second();
            ...
            const double transfer_time = record.get_elapsed_time_second();
