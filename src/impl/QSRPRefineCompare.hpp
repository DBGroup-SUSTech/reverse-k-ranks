//
// Created by bianzheng on 2023/5/19.
//

#ifndef REVERSE_KRANKS_QSRPREFINECOMPARE_HPP
#define REVERSE_KRANKS_QSRPREFINECOMPARE_HPP
...
#include "alg/DiskIndex/ComputeLEMP.hpp"
...
namespace ReverseMIPS::QSRPRefineCompare {
    ...
    class Index : public BaseIndex {
        ...
        //rank bound search
        NormalLinearRegressionLP rank_bound_ins_;
        //rank search
        SampleSearch rank_ins_;
        //read disk
        BaseCompute *disk_ins_;

        std::string dataset_name_;
        int n_thread_;

        int vec_dim_, n_data_item_, n_user_;
        double total_retrieval_time_, inner_product_time_, rank_bound_time_, prune_user_time_, refine_user_time_, pred_refinement_time_;
    ...
    public:
        ...
        Index(
                ...
                NormalLinearRegressionLP &rank_bound_ins,
                //rank search
                SampleSearch &rank_ins,
                //disk index
                BaseCompute *disk_ins,
                //general retrieval
        ...
        Retrieval(const VectorMatrix &query_item, const int &topk, const int &n_execute_query,
            ...
            for (int queryID = 0; queryID < n_query_item; queryID++) {
                if (dataset_name_ == "amazon-home-kitchen-150d" && queryID == 435 && topk == 200) {
                    continue;
                }
...
//                spdlog::error("refine user size {}", refine_user_size);
                ...
                //calculate IP
                inner_product_record_.reset();
                ip_bound_ins_.ComputeRemainDim(query_vecs, svd_ins_.transfer_user_, prune_l_, result_l_, queryIP_l_,
                                               n_user_);
                ...
                pred_refinement_time_ += tmp_pred_refinement_time;
...
//                for (int candID = 0; candID <= n_refine_user; candID++) {
////                    const int userID = 956;
////                    if (queryID == 97 && disk_ins_.user_topk_cache_l_[candID].userID_ == userID) {
////                        spdlog::error("appear queryID {}, userID {}, rank {}, queryIP {:.2f}",
////                                      queryID, userID, disk_ins_.user_topk_cache_l_[candID].rank_, disk_ins_.user_topk_cache_l_[candID].queryIP_);
////                    }
//                    if (queryID == 97) {
//                        spdlog::error("appear queryID {}, userID {}, rank {}, queryIP {:.2f}",
//                                      queryID, disk_ins_.user_topk_cache_l_[candID].userID_, disk_ins_.user_topk_cache_l_[candID].rank_, disk_ins_.user_topk_cache_l_[candID].queryIP_);
//                    }
//                }
    ...
    BuildIndex(const VectorMatrix &data_item, const VectorMatrix &user, const char *dataset_name,
        ...
        PartDimPartNorm ip_bound_ins(n_user, vec_dim, check_dim, n_thread);
        ...
        //rank search
        SampleSearch rank_ins(index_basic_dir, dataset_name, sample_search_index_name.c_str(),
                              n_sample, true, true,
        ...
        NormalLinearRegressionLP rank_bound_ins(index_basic_dir, dataset_name, regression_index_name, n_sample,
                                                n_sample_query,
        ...
        //disk index
        BaseCompute *disk_ins;
        if (method_name == "QSRPRefineComputeIPBound") {
//            ComputeAllIPBound tmp_disk_ins(user, data_item, check_dim);
//            disk_ins = &tmp_disk_ins;
            disk_ins = new ComputeAllIPBound(user, data_item, check_dim, n_thread);
        } else if (method_name == "QSRPRefineComputeAll") {
//            ComputeAll tmp_disk_ins(user, data_item);
//            disk_ins = &tmp_disk_ins;
            disk_ins = new ComputeAll(user, data_item, n_thread);
        } else if (method_name == "QSRPRefineLEMP") {
//            ComputeLEMP tmp_disk_ins(user, data_item);
//            disk_ins = &tmp_disk_ins;
            disk_ins = new ComputeLEMP(user, data_item, n_thread);
        } else {
            spdlog::error("no such method_name in QSRPRefineCompare");
        }
...
#endif //REVERSE_KRANKS_QSRPREFINECOMPARE_HPP
