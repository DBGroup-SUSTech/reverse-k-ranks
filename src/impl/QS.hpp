//
// Created by bianzheng on 2023/5/22.
//

#ifndef REVERSE_KRANKS_QS_HPP
#define REVERSE_KRANKS_QS_HPP
    ...
    public:
        ...
        Retrieval(const VectorMatrix &query_item, const int &topk, const int &n_execute_query,
            ...
            for (int queryID = 0; queryID < n_query_item; queryID++) {
                ...
                std::memcpy(query_vecs, old_query_vecs, vec_dim_ * sizeof(float));
...
//                }
                ...
                const double &memory_index_time = tmp_inner_product_time + tmp_second_rank_bound_time +
        ...
        PerformanceStatistics(const int &topk) override {
            ...
            sprintf(buff,
                    "top%d retrieval time: total %.3fs\n\tinner product %.3fs, rank bound %.3fs, prune user %.3fs, refine user %.3fs\n\ttotal ip cost %ld, total refine ip cost %ld, total refine user %ld, IP bound prune ratio %.4f, average compute item %.3f",
    ...
    BuildIndex(const VectorMatrix &data_item, VectorMatrix &user, const char *dataset_name,
...
//        ip_bound_ins.Preprocess(svd_ins.transfer_user_);
        ...
        const std::string method_name = "QS";
        ...
        std::unique_ptr<Index> index_ptr = std::make_unique<Index>(rank_ins,
                                                                   disk_ins, user,
                                                                   n_user, n_data_item, vec_dim, n_thread);
        return index_ptr;
    }

}
#endif //REVERSE_KRANKS_QS_HPP
