...
def plot_figure(*, fname: str, dataset_name: str, ylim: list,
    ...
    for method_i, key in enumerate(method_m.keys()):
        ...
        y_name = key
        # x_l = df[x_name]
        x_l = np.array([0, 0.5, 1, 1.5, 2])
        y_l = df[y_name].to_numpy()
        y_l = y_l / 1000
        ax.plot(x_l, y_l,
    ...
    if test:
        plt.savefig("update_retrieval_{}.jpg".format(dataset_name), bbox_inches='tight', dpi=600)
    else:
        plt.savefig("update_retrieval_{}.pdf".format(dataset_name), bbox_inches='tight')


if __name__ == "__main__":
    is_test = True

    name_m = {'csv_x': 'memory_capacity', 'fig_x': 'Update Product Percentage (%)',
